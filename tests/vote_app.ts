...
const expectAnchorErrorCode = (err: unknown, expectedCode: string) => {
  ...
  const actualCode =
    anyErr?.error?.errorCode?.code ?? anyErr?.errorCode?.code ?? anyErr?.code;
...
describe("Testing the voting app", () => {
  ...
  beforeEach(async () => {
    ...
    proposalPda = findPda(program.programId, [
      anchor.utils.bytes.utf8.encode(SEEDS.PROPOSAL),
      Buffer.from([PROPOSAL_ID]),
    ]);
  ...
  describe("2.Buy Tokens", () => {
    ...
    it("2.2 buys tokens for voter!", async () => {
      const tokenBalanceBefore = (
        await getAccount(connection, voterTokenAccount)
      ).amount;
  ...
  describe("3.Voter", () => {
    it("3.1 registers voter!", async () => {
      ...
      expect(voterAccountData.voterId.toBase58()).to.equal(
  ...
  describe("4.Proposal Registration", () => {
    it("4.1 registers proposal!", async () => {
      ...
      await program.methods
        .registerProposal(proposalInfo, deadLineTime, stakeAmount)
        .accounts({
        ...
        .signers([proposalCreatorWallet])
        .rpc();
      const proposalAccountData = await program.account.proposal.fetch(
        proposalPda,
      );
      const proposalCounterAccountData =
        await program.account.proposalCounter.fetch(proposalCounterPda);
      expect(proposalCounterAccountData.proposalCount).to.equal(2);

      expect(proposalAccountData.authority.toBase58()).to.equal(
        proposalCreatorWallet.publicKey.toBase58(),
      );
      expect(proposalAccountData.deadline.toString()).to.equal(
      ...
      expect(proposalAccountData.proposalInfo.toString()).to.equal(
  ...
  describe("5.Casting Vote", () => {
    it("5.1 casts vote!", async () => {
      const stakeAmount = new anchor.BN(1000);
      await program.methods
        .proposalToVote(PROPOSAL_ID, stakeAmount)
        .accounts({
          authority: voterWallet.publicKey,
  ...
  describe("8.Close Voter Account", () => {
    it("8.1 Should close voter account one after deadline and recover rent", async () => {
      ...
      const voterBalanceBefore = await connection.getBalance(
        voterWallet.publicKey,
      );
      ...
      const voterBalanceAfter = await connection.getBalance(
        voterWallet.publicKey,
      );
